"""
OneBQF.py: 1-Bit Quantum Filter using exact two-level unitary decomposition.

This module provides the ``OneBQF`` class, an alternative implementation
of the 1-Bit HHL / Quantum Filter algorithm kept alongside
:mod:`lhcb_velo_toy.solvers.quantum.one_bit_hhl`. It implements
controlled time evolution exactly via Givens (two-level) rotations,
which avoids spectral leakage ("ghost couplings") for arbitrary Hamming
distance interaction pairs and supports running with an IBM noise model.

Notes
-----
The canonical, package-exposed implementation is
:class:`lhcb_velo_toy.solvers.quantum.one_bit_hhl.OneBitHHL`. This module
is retained as a reference / development variant and is not re-exported
...
class OneBQF:
    """
    1-Bit Quantum Filter with exact controlled-evolution via Givens rotations.

    Targets matrices ``A`` with constant diagonal ``c·I + B``, where
    ``B`` encodes off-diagonal interactions. The controlled
    ``e^{-iHt}`` is implemented exactly using two-level unitary
    decompositions, making the circuit valid for any pattern of
    interaction pairs and avoiding the Trotter error inherent to a
    first-order product formula.

    Parameters
    ----------
    matrix_A : numpy.ndarray
        System matrix. Must (after padding) have a constant diagonal.
    vector_b : numpy.ndarray
        Right-hand side vector. Will be normalised internally.
    num_time_qubits : int, default 1
        Number of phase-estimation qubits (the "1-bit" of the name).
    shots : int, default 1024
        Number of measurement shots.
    debug : bool, default False
        If True, print the matrix analysis (interaction pairs etc.).

    Attributes
    ----------
    A : ndarray
        The (possibly padded and symmetrised) system matrix.
    vector_b : ndarray
        The normalised RHS vector.
    diagonal_val : float
        Constant diagonal value ``c`` of ``A``.
    interaction_pairs : list[tuple[int, int]]
        Indices ``(i, j)`` with ``i < j`` and non-zero off-diagonal
        couplings in ``B = c·I − A``.
    t : float
        Evolution time, ``π / c``.
    circuit : qiskit.QuantumCircuit | None
        Built circuit (after :meth:`build_circuit`).
    ...
    ------
    ValueError
        If ``A`` does not have a constant diagonal after padding.
    """

    def __init__(self, matrix_A, vector_b, num_time_qubits=1, shots=1024, debug=False):
        """Pad / symmetrise inputs, validate diagonal, extract interaction pairs."""
        A = matrix_A
    ...
    def _apply_direct_controlled_u(self, qc, control_qubit, target_qubits, power, inverse=False):
        ...
        qc : qiskit.QuantumCircuit
            Circuit being built (modified in-place).
        control_qubit : Qubit
            Time-register qubit acting as control.
        target_qubits : list[Qubit]
            System register qubits.
        power : int
            Power of the unitary, ``2**i`` in standard QPE.
        inverse : bool, default False
            If True, apply the Hermitian conjugate (used during
            uncomputation).
        """
    ...
    def apply_controlled_u(self, qc, control_qubit, target_qubits, power, inverse=False):
        """Public wrapper around :meth:`_apply_direct_controlled_u`."""
    ...
    def build_circuit(self):
        """
        Build the full 1-Bit Quantum Filter circuit.

        Returns
        -------
        qiskit.QuantumCircuit
            Constructed circuit, also stored on :attr:`circuit`.

        Notes
        -----
        Steps:

        1. Hadamard the system register (uniform superposition as ``|b⟩``).
        2. Phase estimation with a single time qubit.
        3. Conditional CNOT onto the ancilla (eigenvalue inversion).
    ...
    def get_solution(self, counts=None):
        """
        Decode the solution vector ``x`` from measurement counts.

        Post-selects on ancilla = ``|1⟩`` and converts the resulting
        probability distribution into amplitudes.

        Parameters
        ----------
        counts : dict[str, int], optional
            Measurement counts. If supplied, overrides :attr:`counts`.

        Returns
        -------
        ndarray
            Unit-normalised solution vector in the (padded) system
            basis, or a zero vector of length ``original_dim`` if no
            successful post-selection occurred.

        Raises
        ------
        ValueError
            If no measurement results are available.
        """
