...
\begin{frame}{End-to-End Pipeline}
  ...
  \begin{tikzpicture}[
    ...
    \node[stage, fill=lhcbgreen!12] (geo)
      {1.\ Define\\Geometry};
    \node[stage, fill=lhcbgreen!12, right=0.7cm of geo] (gen)
      {2.\ Generate\\Events};
    \node[stage, fill=lhcbgreen!12, right=0.7cm of gen] (noise)
      {3.\ Add\\Noise};

    % Row 2 — solving
    \node[stage, fill=lhcbblue!12, right=0.7cm of noise] (ham)
      {4.\ Build\\Hamiltonian};
    \node[stage, fill=lhcbblue!12, below=0.8cm of ham] (solve)
      {5.\ Solve\\$Ax = b$};

    % Row 3 — reconstruction + analysis
    \node[stage, fill=lhcbblue!12, left=0.7cm of solve] (reco)
      {6.\ Extract\\Tracks};
    \node[stage, fill=lhcbred!12, left=0.7cm of reco] (val)
      {7.\ Validate};
    \node[stage, fill=lhcbred!12, left=0.7cm of val] (plot)
      {8.\ Visualise};
    ...
    % Data labels
    \node[data, below=1pt of geo]   {PlaneGeometry};
    \node[data, below=1pt of gen]   {Event};
    \node[data, below=1pt of noise] {noisy Event};
    \node[data, above=1pt of ham]   {A (sparse), b};
    \node[data, right=2pt of solve, anchor=west] {x (solution)};
    \node[data, below=1pt of reco]  {list[Track]};
    \node[data, below=1pt of val]   {metrics};

    % Layer labels
    \begin{scope}[on background layer]
      \node[draw=lhcbgreen!50, fill=lhcbgreen!3, rounded corners,
            fit=(geo)(gen)(noise), inner sep=6pt,
            label={[font=\scriptsize\bfseries\color{lhcbgreen}]below:generation}] {};
      \node[draw=lhcbblue!50, fill=lhcbblue!3, rounded corners,
            fit=(ham)(solve)(reco), inner sep=6pt,
            label={[font=\scriptsize\bfseries\color{lhcbblue}]below:solvers}] {};
      \node[draw=lhcbred!50, fill=lhcbred!3, rounded corners,
            fit=(val)(plot), inner sep=6pt,
            label={[font=\scriptsize\bfseries\color{lhcbred}]below:analysis}] {};
    \end{scope}
  \end{tikzpicture}
  \end{center}
...
# 5. Solve
x = ham.solve_classicaly()    # or solve_conjugate_gradient(A, b)
...
\begin{frame}{Hamiltonian \& Solver Hierarchy}
  ...
  \textbf{Quantum}: HHL and 1-bit HHL — ready for Qiskit integration.
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}{3D Event Display}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \textbf{\color{lhcbred}Old} — \texttt{Event.plot\_segments()}
      \begin{itemize}
        \item Method on data class (671-line file)
        \item Draws detector planes via\\
              \texttt{plot\_surface} + \texttt{point\_on\_bulk}
        \item Red dots, blue segment lines, green ghost crosses
        \item Good visual output; poor separation
      \end{itemize}
    \end{column}
    \begin{column}{0.50\textwidth}
      \textbf{\color{lhcbgreen}New} — \texttt{event\_display.py}
      \begin{itemize}
        \item Standalone functions in \texttt{analysis/plotting/}
        \item \texttt{plot\_event\_3d(event, ...)}
        \item \texttt{plot\_segments\_3d(segments, event, ...)}
        \item Same \texttt{plot\_surface} + \texttt{point\_on\_bulk} rendering,\\
              now in a reusable \texttt{\_draw\_detector\_planes()} helper
        \item Additional: \texttt{plot\_hit\_distribution()},
              performance curves
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{Summary}
  \begin{enumerate}
    ...
    \item \textbf{Decoupled visualisation} reusing the original detector-plane
          rendering code

    \item \textbf{Quantum-ready} scaffold (\texttt{solvers/quantum/})
...
% ------------------------------------------------------------------
\begin{frame}{Call for Use Cases}
  \begin{center}
    \large
    We would like to collect \textbf{use cases} from potential users!
  \end{center}

  \vspace{6pt}
  \begin{itemize}
    \item What detector configurations would you like to simulate?
    \item Which physics scenarios are most relevant to your work?
    \item What output formats or analysis hooks do you need?
    \item Are there specific quantum-solver benchmarks you want to run?
  \end{itemize}
