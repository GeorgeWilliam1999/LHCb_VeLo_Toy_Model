...
class EventValidator:
    ...
    def match_tracks(
        self,
        completeness_min: float = 0.3,          # GOOD and ghost threshold now completeness-based
        purity_min: Optional[float] = 0,                # optional extra gate for GOOD if enforce_purity=True
        min_rec_hits: Optional[int | float] = None,
        min_shared_hits: int = 0,
        enforce_purity: bool = False,           # keep optional purity gate for GOOD
        ) -> Tuple[Dict[int, Match], Dict[int, List[int]], List[int]]:
        """
        Build best matches and the pre-clone truth→recs index.
        GOOD acceptance is driven by completeness ≥ completeness_min.
        """
        # Reco-level gate; by default keep all reconstructed tracks
        if min_rec_hits is None:
            min_rec_hits = completeness_min * self.max_hits_in_recon_truth
        else:
        ...
        for rec_id, R_i in self.rec_track_hits.items():
            ...
            # GOOD acceptance: completeness is the primary criterion
            passes_comp   = (m.completeness >= completeness_min)
            # print(f"  Best match: truth {m.best_truth_id} |R∩T|={m.correct_hits}, purity={m.purity:.3f}, completeness={m.completeness:.3f}")
            passes_purity = (m.purity >= purity_min) if enforce_purity else True
            # passes_shared = (m.correct_hits >= min_shared_hits)


            if (m.best_truth_id is not None) and passes_comp and passes_purity:
                m.accepted = True
    ...
    def compute_metrics(
        self,
        completeness_min: float = 0.7,       # GOOD and ghost threshold (completeness-based)
        purity_min: float = 0,             # optional extra gate for GOOD if enforce_purity=True
        min_rec_hits: Optional[int | float] = None,
        min_shared_hits: int = 0,
        enforce_purity: bool = False,
    ) -> dict:
        """
        GOOD (efficient) tracks: completeness ≥ completeness_min (after clone removal).
        GHOSTS: completeness < completeness_min (among candidates, excluding clones).
        """
        ...
        # GHOSTS: completeness < threshold (completeness-based), among candidates and not clones
        ghost_rec_ids = [
            rid for rid in rec_best
            if rec_best[rid].candidate
            and (rec_best[rid].completeness < float(completeness_min))
